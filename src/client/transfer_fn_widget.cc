...
#include "embedded_colormaps.hpp"
...
bool TransferFunctionWidget::select_colormap(const std::string &name)
{
	auto it = std::find_if(colormaps.begin(), colormaps.end(),
						   [&](auto &colormap) { return colormap.name == name; });
