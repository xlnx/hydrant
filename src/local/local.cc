...
int main( int argc, char **argv )
{
	...
	a.add<string>( "mod_path", 0, "renderer module path", false, "mods" );
	...
	auto mpath = FilePath( a.get<string>( "mod_path" ) );
	ensure_dir( mpath.resolved() );
	auto mod_paths = fs::open( mpath.resolved() ).listFiles();
	vector<unique_ptr<vm::Library>> mods( mod_paths.size() );
	transform( mod_paths.begin(), mod_paths.end(),
			   mods.begin(), [&]( auto &path ) {
				   return unique_ptr<vm::Library>(
					 new vm::Library( mpath.resolve( path ).resolved() ) );
			   } );
